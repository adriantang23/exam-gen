...
1. Reads structured JSON files from the `parsed_output/` directory (produced by
   `DocumentOrganizer`).
2. Iterates independently through each section (e.g., each homework question)
   and calls OpenAI with a custom system prompt provided by the user.
3. Utilises the function-calling feature to ask ChatGPT to respond with a
   structured `create_question` call containing:
   * `question`  – the newly generated mock question (string)
   * `probability` – integer 1-100 likelihood it appears on the real exam
...
6. Designed to be cost-efficient: one API call **per original question** so you
...
class LLMExamGenerator:
    ...
    def _generate_for_category(
        ...
        for doc_name, category, section_idx, section_text in all_sections:
            try:
                function_call = self._prompt_llm(section_text, system_prompt)
                results.append(
                    {
    ...
    def _prompt_llm(self, section_text: str, system_prompt: str) -> Dict[str, Any]:
